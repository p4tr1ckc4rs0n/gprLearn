...
for rx in range(1, nrx + 1):
    ...
    if len(args.outputs) == 1:
        ...
        if args.fft:
            ...
            ax1.set_title("A-scan")
            ax1.set_xlabel('Time [s]')
            ax1.set_ylabel(outputtext + ' field strength [V/m]',fontsize=50)
    ...
    # If multiple outputs required, create all nine subplots and populate only the specified ones
        ...
        for output in args.outputs:
            ...
            #ax.set_ylim([-15, 20])
                ...
                ax.plot(time, outputdata,'r', lw=2, label=outputtext)
                ax.set_ylabel(outputtext + ', field strength [V/m]',fontsize=50)
                ax.set_title('A-scan',fontsize=20)
            #ax.set_ylim([-15, 20])
            elif output == 'Hx':
